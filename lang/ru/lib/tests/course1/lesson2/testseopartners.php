<?php
$MESS['INTERVOLGA_EDU.COURSE_1_LESSON_2_KEYWORDS'] = 'В разделе Партнерам должно быть заполненно от трех ключевых слов';
$MESS['INTERVOLGA_EDU.COURSE_1_LESSON_2_DESCRIPTION'] = 'В разделе Партнерам должно быть заполненно описание';
$MESS['INTERVOLGA_EDU.COURSE_1_LESSON_2_KEYWORDS_PAGE_BECOME_PARTNERS'] = 'Для страницы "Как стать партнером" должно быть заполненно от трех ключевых слов';
$MESS['INTERVOLGA_EDU.COURSE_1_LESSON_2_TITLE_PAGE_BECOME_PARTNERS'] = 'Для страницы "Как стать партнером" должен быть установлен заголовок';
$MESS['INTERVOLGA_EDU.COURSE_1_LESSON_2_DESCRIPTION_PAGE_BECOME_PARTNERS'] = 'Для страницы "Как стать партнером" должно быть заполненно описание';
$MESS['INTERVOLGA_EDU.COURSE_1_LESSON_2_NOT_FOUND_PARTNERS_HOW_BECOME_PAGE'] = 'Как стать партнером';